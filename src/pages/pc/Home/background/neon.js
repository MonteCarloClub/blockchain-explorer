export function neonPath(canvas, config = {}) {
    ...
    Line.prototype.step = function () {
        ...
        if (Math.random() < opts.sparkChance) {
            const xx = this.x + Math.random() * opts.sparkDist * (Math.random() < .5 ? 1 : -1) - opts.sparkSize / 2;
            const yy = this.y + Math.random() * opts.sparkDist * (Math.random() < .5 ? 1 : -1) - opts.sparkSize / 2;
